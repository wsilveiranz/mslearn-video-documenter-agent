...
## Phase 3: Cloud Services & Quality
...
#### 3.1 Azure Video Indexer Integration
...
#### 3.7 Content Mentor Integration Guide
- Document how to use Content Mentor alongside the agent
- Test generated documents with Content Mentor validation
- Add tips for users on post-generation refinement workflow
...
#### 4.1 Screenshot Annotation Engine
...
| **Phase 3** | Cloud processing + quality gates | Azure Video Indexer, Azure Speech, Blob Storage |
| **Phase 4** | Production deployment + polish | Docker, Azure Container Apps, Pillow |
...
| Video Indexer shot types (Wide/Medium/Close-up) may not apply to screen recordings | Phase 3 | Ignore shot type tags; use scene/keyframe data only |
...
| Evaluate Agent scoring may be unreliable (Doc-Kit has this issue too) | Phase 3 | Use simple rule-based checks alongside LLM evaluation; set conservative thresholds |
